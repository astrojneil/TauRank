...
def calcRankTau(directory, runName, runNumber, ions, velocityBin, frameVel):
	...
	plot.save()
        ...
        writeFile = open('../rankTau'+ion['ionfolder']+'rankTau'+runName+'_v'+str(int(round(velocityBin/1e5, 0)))+'.txt', 'w')
...
### run though functions!
    for run in runList:
        ...
        for v in range(len(velBins)): #velocity = velBins[v]
            ...
            tauList = calcRankTau(run['Dir'], run['Name'], run['f_list'][v], ionList, velBins[v], velFrames[v])
