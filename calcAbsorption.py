...
def calcAbsorb(directory, runName, runNumber, ions, velocityBin, frameVel):
    ...
    #file to save taus; write header
    saveCharTaus = open('charTaus/charTaus'+runName+'_v'+str(round(velocityBin, 3))+'.txt', 'w')
    for ion in ions:
        saveCharTaus.write(ion['ion']+', ')
    ...
    for ion in ions:
        ...
        #t = yt.ParticlePlot(test, ('no_colDepth', 'px'), ('no_colDepth', 'py'), ('no_colDepth', 'tau'))
        #t.set_ylim(-1.2*kpc, 1.2*kpc)
        #t.set_xlim(-1.2*kpc, 1.2*kpc)
        #t.save()
        ...
        print(runName)
        print(ion['ion'])
        print('Characteristic Tau:')
        print(np.average(characteristicTau))
    ...
    #write taus to file
    tauListString = ", ".join(map(str, absorbFracs))
    saveCharTaus.write(tauListString)
    saveCharTaus.close()
...
### run though functions!
    for run in runList:
        ...
        saveData = open('absorptionFits/absorptionFits'+run['Name']+'_2.txt', 'w')
        ...
        for v in range(len(velBins)): #velocity = velBins[v]
            ...
            tauList = calcAbsorb(run['Dir'], run['Name'], run['f_list'][v], ionList, velBins[v], velFrames[v])
